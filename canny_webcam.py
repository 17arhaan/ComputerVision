...
def reset_image():
    global modified_image, apply_canny, original_image
    if original_image:
        modified_image = original_image.copy()
        update_modified_image()

        # Reset live feed as well
        apply_canny = False
        # Reset the live feed display
        update_webcam_feed()
...
def update_webcam_feed():
    ret, frame = cap.read()
    if ret:
        # Resize frame to square aspect ratio
        height, width = frame.shape[:2]
        min_side = min(height, width)
        start_x = (width - min_side) // 2
        start_y = (height - min_side) // 2
        frame_square = frame[start_y:start_y + min_side, start_x:start_x + min_side]
        ...
        if apply_canny:
            ...
            frame_gray = cv2.cvtColor(frame_square, cv2.COLOR_BGR2GRAY)
            sigma = float(sigma_entry.get())
            smoothed_image_np = gaussian_filter(frame_gray, sigma=sigma)
            edges = feature.canny(smoothed_image_np)
            # Convert edges to an image
            frame_rgb = np.uint8(edges * 255)
...
webcam_label = tk.Label(webcam_frame, width=320, height=320)  # Set width and height to be square
...
canny_edge_button = tk.Button(button_frame, text="Canny Edge Detection", command=apply_canny_on_live_feed)
